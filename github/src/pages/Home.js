...
const Home = () => {
  const [username, setUsername] = useState("TarannumAfreen");
  ...
  const clearUserData = () => {
    setUserData(null);
    setUsername("Tarannum"); // Reset to default user when clearing
  };
    ...
    <div className="bg-gradient-to-r from-pink-200 to-green-200 min-h-screen">
      ...
      <div className="mt-10 p-4 text-black rounded-md w-80 md:w-1/2 lg:w-1/3 mx-auto relative">
        ...
        {!userData && (
          <>
            <img
              src={`https://api.github.com/users/${username}/avatar`}
              alt="Default Avatar"
              className="rounded-full w-32 h-32 mx-auto"
            />
